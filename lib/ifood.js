...
export function getCurrentSaoPauloTime() {
...
export function getCurrentSaoPauloTimeStart() {
  const { minutes } = getCurrentSaoPauloTime();
  const hours = String(Math.floor(minutes / 60)).padStart(2, '0');
  const remainingMinutes = String(minutes % 60).padStart(2, '0');
  return `${hours}:${remainingMinutes}:00`;
}

function parseShiftStart(start) {
  const [hours, minutes] = String(start || '').split(':').map(Number);
  if (!Number.isInteger(hours) || !Number.isInteger(minutes)) return null;
  return hours * 60 + minutes;
}

export function isOpeningHoursOpen(shifts) {
  const { dayOfWeek, minutes } = getCurrentSaoPauloTime();
  const dayIndex = DAYS_OF_WEEK.indexOf(dayOfWeek);

  return shifts.some((shift) => {
    const start = parseShiftStart(shift.start);
    const duration = Number(shift.duration);
    if (start === null || !Number.isFinite(duration) || duration <= 0) return false;

    if (shift.dayOfWeek === dayOfWeek && minutes >= start && minutes < start + duration) {
      return true;
    }

    const previousDay = DAYS_OF_WEEK[(dayIndex + DAYS_OF_WEEK.length - 1) % DAYS_OF_WEEK.length];
    return shift.dayOfWeek === previousDay && start + duration > 1440 && minutes < start + duration - 1440;
  });
}
...
function parseSaoPauloDateTime(value) {
  ...
  const normalized = /(?:Z|[+-]\d{2}:\d{2})$/.test(text) ? text : `${text}-03:00`;
  const timestamp = new Date(normalized).getTime();
  return Number.isNaN(timestamp) ? null : timestamp;
}

export function isInterruptionActive(interruption, now = Date.now()) {
  const start = parseSaoPauloDateTime(interruption?.start);
  const end = parseSaoPauloDateTime(interruption?.end);
  return start !== null && end !== null && now >= start && now < end;
}

export async function getInterruptions(merchantId) {
  const res = await ifoodFetch(`/merchant/v1.0/merchants/${merchantId}/interruptions`);
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Erro ao buscar interrupções (${res.status}): ${text}`);
  }
  const data = await res.json();
  return Array.isArray(data) ? data : data?.interruptions || [];
}

export async function createInterruption(merchantId, description) {
  const start = formatSaoPauloDateTime(new Date());
  const end = formatSaoPauloDateTime(new Date(Date.now() + 24 * 60 * 60 * 1000));
  const res = await ifoodFetch(`/merchant/v1.0/merchants/${merchantId}/interruptions`, {
    method: 'POST',
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Erro ao fechar a loja (${res.status}): ${text}`);
  }
}

export async function deleteInterruption(merchantId, interruptionId) {
  const res = await ifoodFetch(`/merchant/v1.0/merchants/${merchantId}/interruptions/${interruptionId}`, {
    method: 'DELETE'
  });
  ...
  if (!res.ok) {
    ...
    try {
      ...
      // Mantém a mensagem original quando o iFood não retornar JSON.
    }
    const error = new Error(`Erro ao abrir a loja (${res.status}): ${text}`);
    error.code = code;
    throw error;
  }
}
...
export async function getOpeningHours(merchantId) {
  ...
  const data = await res.json();
  // A API pode retornar um objeto ou uma lista, dependendo do ambiente.
  return Array.isArray(data) ? data[0]?.shifts || [] : data?.shifts || [];
}

export async function setOpeningHours(merchantId, shifts) {
  const res = await ifoodFetch(`/merchant/v1.0/merchants/${merchantId}/opening-hours`, {
    method: 'PUT',
    body: JSON.stringify({ storeId: merchantId, shifts })
  });
  if (!res.ok) {
    const text = await res.text();
